function [S, timeVect, distancesAll, additionalData] = KiNeT( scores, dt )
...
if size(vectorAngleMarginal,2) == 1
    meanVectorAngleMarginal = vectorAngleMarginal';
else
     meanVectorAngleMarginal = nanmean(vectorAngleMarginal');
end
additionalData.meanVectorAngle = meanVectorAngleMarginal;
additionalData.spaceAngle = spaceAngle;
...
if plotData
    ...
    for i = nCond:-1:1
        lc(i) = line(timeVect,minDistInd(indVect,i)*dt, ...
            'Color',lineColor(i,:));
    ...
    lcN = [];
    for i = nCond:-1:1
        lcN(i) = line(timeVect,distancesAll(indVect,i), ...
            'Color',lineColor(i,:));
    end
    setLineColors(lcN);
    set(gca,'visible','on');
