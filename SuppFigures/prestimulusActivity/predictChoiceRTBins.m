%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% created by Tian on Feb.4th 2022: predict choice using different reaction time bins 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% save([savePath 'decoderbyRTBins.mat'], 'classifier');


%% Sanity check: plot the figure of accuracy (1 day)

% day = 2;
% 
% for ip = 1 : size(classifier(day).accuracy,2)
%      figure;
%      plot(classifier(day).accuracy(:,ip));
% 
%      title(['RT between ' mat2str(RTBin(ip, :))]);
%      
% 
% end
...
for ip = 1:size(aveAccuracy,2)
    ...
    [data_mean(ip,:), upperBd(ip,:), lowerBd(ip,:)] = plot_areaerrorbar(validData', options);
end
...
plot([0,0], [ylimit,yUp], 'color', [0.5 0.5 0.5], 'linestyle', '--', 'linewidth',3)
...
vLimits = [ylimit,yUp];
vTickLocations = linspace(ylimit, yUp, 3); %[ylimit ylimit + (1-ylimit)/2 1];
vLabOffset = 180;
vAxisOffset =  -650;
vLabel = "Accuracy"; 
...
% print('-painters','-depsc',['~/Desktop/', 'predictChoiceRTBins','.eps'], '-r300');
