function [beta, total_mse, t, mse_t, rsquareT, yPred] = rrr(X, Y, varargin)
            ...
            if length(varargin{i-1}) == 2
                ...
                for j=1:1:s
                    for k=1:1:K
                        ...
                        [mse_k(k), rsquareV(k)] = compute_mse(bb, X(cv(k).test, :), Y(cv(k).test, :));
                    end
                    mse_t(j) = mean(mse_k);
                    rsquareT(j) = mean(rsquareV);
                end
...
[total_mse,~,yPred] = compute_mse(beta, X, Y);
    ...
    function [mse, rsquare, Y_pred] = compute_mse(B, X, Y)
        Y_pred = [ones(size(X, 1), 1) X]*B';
        er = (Y - Y_pred).^2;
        totV = sum([Y(:) - nanmean(Y(:))].^2);
        mse = mean(er(:));
        rsquare = 1 - sum(er(:))./totV;
    end
end
